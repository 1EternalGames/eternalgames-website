            ...
            <div className={`${styles.timelineCard} ${autoHeight ? styles.autoHeight : ''} ${variant === 'homepage' ? styles.homepage : ''}`} style={{ position: 'relative' }}>
                ...
                <Link href={mainHref} className="no-underline block h-full" onClick={handleClick} prefetch={false} style={{ position: 'relative', zIndex: 1 }}>
                    ...
                    <div className={styles.imageFrame}>
                        {isReleased ? <div className={`${styles.statusBadge} ${styles.released}`}> <div style={{width:'var(--tl-cal-icon-size)',height:'var(--tl-cal-icon-size)'}}><svg viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.052-.143z" clipRule="evenodd" /></svg></div> <span>صدرت</span> </div> : <div className={`${styles.statusBadge} ${styles.upcoming}`}> <div style={{width:'var(--tl-cal-icon-size)',height:'var(--tl-cal-icon-size)'}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg></div> <span>قادمة</span> </div>}
