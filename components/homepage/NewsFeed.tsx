...
const containerVariants = {
    hidden: { opacity: 0 },
    visible: { opacity: 1, transition: { staggerChildren: 0.07, delayChildren: 0.4 } }
};
...
export default function NewsFeed({ pinnedNews, newsList }: {
    ...
    return (
        ...
        >
            <motion.div variants={itemVariants} className={styles.pinnedNewsSection}>
                ...
                {pinnedNews.map(item => (
                    <Link href={`/news/${item.slug}`} key={item.id} className={`${styles.pinnedNewsItem} no-underline`}>
            ...
            <motion.div variants={itemVariants} className={styles.newsList}>
                {newsList.map(item => (
                    <Link href={`/news/${item.slug}`} key={item.id} className={`${styles.newsListItem} no-underline`}>
                        <div className={styles.newsListThumbnail}>
                            <Image src={item.imageUrl} alt={item.title} fill sizes="60px" placeholder="blur" blurDataURL={item.blurDataURL} style={{ objectFit: 'cover' }} />
                        </div>
