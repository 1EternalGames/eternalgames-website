    ...
    return (
        ...
        >
            <div className={styles.topArticleImage}>
                <Image src={article.imageUrl} alt={article.title} fill sizes="(max-width: 768px) 90vw, 30vw" placeholder="blur" blurDataURL={article.blurDataURL} style={{ objectFit: 'cover' }} />
            </div>
            <div className={styles.topArticleContent}>
                ...
                <div className={styles.topArticleMeta}>
                    <CreatorCredit label="" creators={article.authors} />
                </div>
            </div>
        </motion.div>
    );
...
export default function ArticlesFeed({ topArticles, latestArticles }: {
    ...
    return (
        ...
        >
            <motion.div className={styles.topArticlesGrid} variants={itemVariants}>
                {topArticles.map(article => <TopArticleCard key={article.id} article={article} />)}
            </motion.div>

            <motion.div variants={itemVariants} style={{textAlign: 'right'}}>
                <span className={styles.sectionLabel} style={{marginTop: '4rem'}}>الأحدث</span>
            </motion.div>
            ...
            <motion.div className={styles.latestArticlesList} variants={itemVariants}>
                {latestArticles.map(article => (
                    <div 
                        key={article.id} 
                        className={styles.latestArticleItem} 
                        onClick={(e) => handleItemClick(e, article.slug)}
                    >
                        <div className={styles.latestArticleThumbnail}>
                            <Image src={article.imageUrl} alt={article.title} fill sizes="120px" placeholder="blur" blurDataURL={article.blurDataURL} style={{ objectFit: 'cover' }} />
                        </div>
                        <div className={styles.latestArticleInfo}>
                            <h4 className={styles.latestArticleTitle}>{article.title}</h4>
                            <p className={styles.latestArticleMeta}>
                                <CreatorCredit label="" creators={article.authors} />
                            </p>
                        </div>
                    </div>
