...
defineField({name: 'game', title: 'Game', type: 'reference', to: {type: 'game'}, validation: (Rule: Rule) => Rule.required()}),
defineField({name: 'authors', title: 'Reviewers', type: 'array', of: [{type: 'reference', to: {type: 'reviewer'}}], validation: (Rule: Rule) => Rule.required().min(1)}),
defineField({name: 'designers', title: 'Designers (Optional)', type: 'array', of: [{type: 'reference', to: {type: 'designer'}}]}),
defineField({name: 'mainImage', title: 'Main image', type: 'image', options: {hotspot: true}, validation: (Rule: Rule) => Rule.required()}),
defineField({name: 'score', title: 'Score', type: 'number', validation: (Rule: Rule) => Rule.required().min(0).max(10)}),
defineField({name: 'verdict', title: 'Verdict', type: 'string', validation: (Rule: Rule) => Rule.required()}),
...
defineField({name: 'content', title: 'Content', type: 'blockContent', validation: (Rule: Rule) => Rule.required()}),
