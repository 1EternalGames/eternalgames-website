...
export async function updateUserProfile(formData: FormData) {
    try {
        ...
        await prisma.user.update({
            ...
            data: { name: name || undefined, username: username || undefined, bio: bio.slice(0, 500), twitterHandle, instagramHandle, agePublic, countryPublic },
        });
...
export async function completeOnboardingAction(formData: FormData) {
    try {
        ...
        if (!fullName || fullName.trim() === '') return { success: false, message: 'الاسم الكامل مطلوب.' };
...
export async function changePasswordAction(formData: FormData) {
    try {
        ...
        if (newPassword.length < 8) return { success: false, message: 'كلمة السر الجديدة لا تقل عن ثمانيةِ حروف.' };
...
const validateUsername = (username: string) => {
    ...
    if (!usernameRegex.test(username)) return { success: false, message: 'اسم المستخدم يجب أن يتكون من 3-20 حرفًا إنجليزيًا.' };
    return { success: true, message: '' };
};

export async function checkUsernameAvailability(username: string): Promise<{ available: boolean; message: string }> {
    const validation = validateUsername(username);
    if (!validation.success) return { available: false, message: validation.message };
    const existingUser = await prisma.user.findUnique({ where: { username } });
...
export async function signUp(formData: FormData) {
    ...
    const username = (formData.get('username') as string)?.toLowerCase();
    if (!name || !email || !password || !username) return { success: false, message: 'الحقولُ كلُّها لازمة.' };
    if (!/\S+@\S+\.\S+/.test(email)) return { success: false, message: 'البريد الإلكتروني غير صالح.' };
    if (password.length < 8) return { success: false, message: 'كلمة السر لا تقل عن ثمانيةِ حروف.' };
    const usernameValidation = await checkUsernameAvailability(username);
    if (!usernameValidation.available) return { success: false, message: usernameValidation.message };
